...
with mp_pose.Pose(min_detection_confidence=0.5, min_tracking_confidence=0.5) as pose:
    while cap.isOpened():
        ...
        try:
            ...
            cv2.putText(image, str(round(angle2, 2)),
                        tuple(np.multiply(elbow, [640, 480]).astype(int)),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 2, cv2.LINE_AA)
            cv2.putText(image, str(round(angle3, 2)),
            ...
            cv2.putText(image, "open", (300, 150),
                        cv2.FONT_HERSHEY_PLAIN, 5, (255, 0, 0), 12)
            if upCount == 0:
                angle4 = 10
            else:
                angle4 = 50
